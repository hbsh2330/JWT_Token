...
public class JwtTokenProvider {
    ...
    private final Key key;
        ...
        public JwtTokenProvider() { //JwtTokenProvider 생성자 생성동시
            byte[] keyBytes = KeyGenerator.getKeygen(); //인증키 생성
            this.key = Keys.hmacShaKeyFor(keyBytes); //암호화
            System.out.println("JwtTokenProvider Constructor  Key init: " + key);

        }

    // 유저 정보를 가지고 AccessToken, RefreshToken 을 생성하는 메서드
    public TokenInfo generateToken(Authentication authentication) { //토큰인증은 사용자 정보 받음
        // 권한 가져오기
        ...
        // Access Token 생성
        Date accessTokenExpiresIn = new Date(now + 60*1000); // accessToken 만료기간은 짧을수록 좋음 길면 보안위험 60초후 만료
    ...
    public Authentication getAuthentication(String accessToken) { //authentication 만듬
        // 토큰 복호화
        ...
        userDto.setRole(authorities.toString()); //비밀번호는 추가 하면 안됨!! 중요한 정보는 빼고
