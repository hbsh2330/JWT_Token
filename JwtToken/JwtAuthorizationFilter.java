...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            token = Arrays.stream(request.getCookies()) // 토큰 찾는 로직
                    .filter(cookie -> cookie.getName().equals(JwtProperties.COOKIE_NAME)).findFirst() //이름이 COOKIE_NAME 동일하다면
                    .map(cookie -> cookie.getValue()) //쿠키 찾아서 맵형태로 넣어주고
                    .orElse(null); //아니면 null을 반환
        ...
        if (token != null) {
            try {
                if(jwtTokenProvider.validateToken(token)) {
                    Authentication authentication = getUsernamePasswordAuthenticationToken(token); //토큰을 찾으면
